...
const static int FLANGE = 0;
const static int REVERB = 1;
...
bool CSynthesizer::Generate(double* frame)
{
    ...
    while (m_currentNote < (int)m_notes.size())
    {
        ...
        else if (note->Instrument() == L"SubtractiveSynthesizer")
        {
        ...
        if (instrument != NULL)
        {
            ...
            instrument->SetBeatsPerMinute(GetBeatsPerMinute());
    ...
    {
        // Since we may be removing an item from the list, we need to know in advance
        // what is after it in the list. We keep that node as "next."
        ...
        {
            // Add the resulting frame to the output frame
            for (int c = 0; c < GetNumChannels(); c++)
            {
            ...
            if (instrument->hasEffects(FLANGE))
            {
                double flange[2] = { 0.0, 0.0 };
                mFlangeEffect.Process(frame, flange);

                // Add the flange effect to the frame
                frame[0] += flange[0];
                frame[1] += flange[1];
            }

            if (instrument->hasEffects(REVERB))
            {
                double reverb[2] = { 0.0, 0.0 };
                mReverbEffect.Process(frame, reverb);

                // Add the reverb effect to the frame
                frame[0] += reverb[0];
                frame[1] += reverb[1];
            }

            if (instrument->hasEffects(CHORUS))
            {
                double chorus[2] = { 0.0, 0.0 };
                mChorusEffect.Process(frame, chorus);

                // Add the chorus effect to the frame
                frame[0] += chorus[0];
                frame[1] += chorus[1];
            }

            // Divide by the number of effects, if applicable
            int numEffects = instrument->getNumEffects();
            if (numEffects > 0)
            {
                frame[0] /= numEffects;
                frame[1] /= numEffects;
        ...
        {
            // If we returned false, the instrument is done. Remove it from the list and delete it from memory.
...
void CSynthesizer::XmlLoadInstrument(IXMLDOMNode* xml)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        else if (name == "flange")
        {
            value.ChangeType(VT_I4);
            if (value.intVal == 0)
                m_flangeActivation = true;
        }
        else if (name == "reverb")
        {
            value.ChangeType(VT_I4);
            if (value.intVal == 1)
                m_reverbActivation = true;
        }
        else if (name == "chorus")
        {
            value.ChangeType(VT_I4);
            if (value.intVal == 2)
                m_chorusActivation = true;
        }
    }
...
void CSynthesizer::XmlLoadNote(IXMLDOMNode* xml, std::wstring& instrument)
{
    ...
    if (m_flangeActivation)
    {
        m_notes.back().setEffect(FLANGE);
    }
    if (m_reverbActivation)
    {
        m_notes.back().setEffect(REVERB);
    }
    if (m_chorusActivation)
    {
